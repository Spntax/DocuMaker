...
def convert_to_pdf_libreoffice(source, output_dir, timeout=None) -> str:
    ...
    output = None
    temp_filename = path.join(output_dir,"alt_"+ path.basename(source))
    copy2(source, temp_filename)

    try:
        process = run(
            ['soffice', '--headless', '--convert-to', 'pdf', '--outdir', output_dir, temp_filename],
            stdout=PIPE, stderr=PIPE, timeout=timeout, check=True, shell=True
        )
...
def convert(source, output_dir, soft):
